...
call "POST /api/inventory/check-and-reserve (success)" \
  curl -s -X POST http://localhost:8082/api/inventory/check-and-reserve \
    -H "Content-Type: application/json" \
    -d '{
      "items": {
        "Flour": 2,
...
if [ -n "$ORDER_ID" ]; then
  ...
  call "GET /api/orders/${ORDER_ID}" \
    curl -s "http://localhost:8081/api/orders/${ORDER_ID}"
  ...
  call "PATCH /api/orders/${ORDER_ID}/status?status=CANCELLED" \
    curl -s -X PATCH "http://localhost:8081/api/orders/${ORDER_ID}/status?status=CANCELLED"
else
  echo
...
call "POST /api/kitchen/orders (manual)" \
  curl -s -X POST http://localhost:8083/api/kitchen/orders \
    -H "Content-Type: application/json" \
    -d '{
      "orderId": 1,
...
call "PATCH /api/kitchen/orders/${KITCHEN_ORDER_ID}/ready" \
  curl -s -X PATCH "http://localhost:8083/api/kitchen/orders/${KITCHEN_ORDER_ID}/ready"
...
call "PATCH /api/kitchen/orders/${KITCHEN_ORDER_ID}/delivered" \
  curl -s -X PATCH "http://localhost:8083/api/kitchen/orders/${KITCHEN_ORDER_ID}/delivered"
